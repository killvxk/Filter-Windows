...
namespace CitadelService.Services
{
    public class TrustManager : ITrustManagement
    {
        ...
        public void EstablishTrustWithGit()
        {
            ...
            {
                string gitConfig = Path.Combine(profile, ".gitconfig");

                if (!File.Exists(gitConfig))
                {
                    using (Stream stream = new FileStream(gitConfig, FileMode.Create))
                    using (StreamWriter writer = new StreamWriter(stream))
                    {
                        writer.WriteLine("# This config file was automatically generated by CloudVeil in order for git.exe to just work on our filter.");
                        writer.WriteLine("[http]");
                        writer.WriteLine($"\tsslCAInfo={gitCompatibleBundlePath}");
                    }

                    File.SetAttributes(gitConfig, FileAttributes.Hidden);
                }
                else if(installedGits.Count > 0)
                {
                    string gitExe = installedGits[0];

                    string arguments = $"config --file \"{gitConfig}\" http.sslCAInfo \"{gitCompatibleBundlePath}\"";
